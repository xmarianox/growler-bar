	...
	<div class="primary_block row">
		...
		<div class="pb-right-column col-xs-12 col-sm-4 col-md-3">
			...
			<div class="box-info-product">
				...
				*}
				{if $product->id_category_default == 47 || $product->id_category_default == 63}
				...
				<div class="content_prices">
					<ul>
						<li>
							<p>Individual: <span>{convertPrice price=$productPrice|floatval}</span></p>
							<form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
								<!-- hidden datas -->
								<p class="hidden">
									<input type="hidden" name="token" value="{$static_token}" />
						...
						{foreach from=$subproductos item='subproducto' name='subproductos'}
						<li>
							<p>{$subproducto->description_short|strip_tags}: <span>{convertPrice price=$subproducto->price|floatval}</span></p>
							<form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $subproducto->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
								<!-- hidden datas -->
								<p class="hidden">
									<input type="hidden" name="token" value="{$static_token}" />
									<input type="hidden" name="id_product" value="{$subproducto->id|intval}" id="product_page_product_id" />
									<input type="hidden" name="add" value="1" />
									<input type="hidden" name="id_product_attribute" id="idCombination" value="" />
								</p>
								<button type="submit" name="Submit" class="exclusive">
									<span>{if $content_only && (isset($subproducto->customization_required) && $subproducto->customization_required)}{l s='Customize'}{else}{l s='Comprar'}{/if}</span>
								</button>								
							</form>	
						</li>
						{/foreach}
